...
void	get_player_init_pos_minimap(t_maze *maze)
{
	...
	while(i < maze->map->height)
	{
		...
		while(j < maze->map->width)
		{
			if(is_player(maze->map->layout[i][j]))
				{
					maze->map->mini_player.x = maze->map->offset_2dx + (j * maze->map->block) + (maze->map->block / 2);
					maze->map->mini_player.y = maze->map->offset_2dy + (i * maze->map->block) + (maze->map->block / 2);
